...
with DAG(
    ...
    docker_task = DockerOperator(
        ...
        network_mode="bridge",
        task_id="docker_op_task",
        device_requests=[docker.types.DeviceRequest(device_ids=["0"], capabilities=[['gpu']])],
        xcom_all=False
    )

    process_docker_output_task = PythonOperator(
        task_id="process_docker_output",
        python_callable=process_docker_output
    )

    download_task >> save_to_mysql_task >> mysql_task >> docker_task >> process_docker_output_task
# [END instantiate_dag]
